...
import com.jishiqi.mianshigua.mapper.QuestionBankMapper;
...
public class QuestionBankServiceImpl extends ServiceImpl<QuestionBankMapper, QuestionBank> implements QuestionBankService {
    ...
    public void validQuestionBank(QuestionBank questionBank, boolean add) {
        ...
        Integer priority = questionBank.getPriority();

        // 创建数据时，参数不能为空
        if (add) {
            ThrowUtils.throwIf(StringUtils.isBlank(title), ErrorCode.PARAMS_ERROR,"题库标题不能为空");
            ThrowUtils.throwIf(StringUtils.isBlank(description), ErrorCode.PARAMS_ERROR,"题库描述不能为空");
            ThrowUtils.throwIf(StringUtils.isBlank(picture), ErrorCode.PARAMS_ERROR,"题库图标不能为空");
            ThrowUtils.throwIf(ObjectUtils.isEmpty(priority), ErrorCode.PARAMS_ERROR,"题库优先级不能为空");
        }
        // 修改数据时，有参数则校验
